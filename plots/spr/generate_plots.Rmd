...
lstm_RC_EOI <- ggplot(by_item %>% filter(EOI=="RC" && ROI==1), aes(x=mean_lstm, y=mean)) +
  geom_point() +
  facet_wrap(~EOI) +
  theme_fivethirtyeight()
...
lstm_AA_EOI_LOW <- ggplot(by_item %>% filter(EOI=="LOW" && ROI==0), aes(x=mean_lstm, y=mean)) +
  geom_point()+
  facet_wrap(~EOI)+
  theme_fivethirtyeight()
...
annotate_figure(lstm_arr, top=text_grob("Scatterplot of empirical item EOI by predicted item EOI, Wikitext LSTM", color="#3C3C3C", size=12, family="sans", face="bold"), bottom=text_grob("Predicted EOI", color="#3C3C3C", size=12, family="sans"), left=text_grob("Empirical EOI" , color="#3C3C3C", size=12, family="sans", rot=90))+theme_fivethirtyeight()
...
gpt_RC_EOI <- ggplot(by_item %>% filter(EOI=="RC" && ROI==1), aes(x=mean_gpt2, y=mean)) +
  geom_point() +
  facet_wrap(~EOI) +
  theme_fivethirtyeight()
...
gpt_AA_EOI_LOW <- ggplot(by_item %>% filter(EOI=="LOW" && ROI==0), aes(x=mean_gpt2, y=mean)) +
  geom_point()+
  facet_wrap(~EOI)+
  theme_fivethirtyeight()
...
annotate_figure(gpt_arr, top=text_grob("Scatterplot of empirical item EOI by predicted item EOI, GPT-2", color="#3C3C3C", size=12, family="sans", face="bold"), bottom=text_grob("Predicted EOI", color="#3C3C3C", size=12, family="sans"), left=text_grob("Empirical EOI" , color="#3C3C3C", size=12, family="sans", rot=90))+theme_fivethirtyeight()
...
nosurp_RC_EOI <- ggplot(by_item %>% filter(EOI=="RC" && ROI==1), aes(x=mean_nosurp, y=mean)) +
  geom_point() +
  facet_wrap(~EOI) +
  theme_fivethirtyeight()
...
nosurp_AA_EOI_LOW <- ggplot(by_item %>% filter(EOI=="LOW" && ROI==0), aes(x=mean_nosurp, y=mean)) +
  geom_point()+
  facet_wrap(~EOI)+
  theme_fivethirtyeight()
...
annotate_figure(nosurp_arr, top=text_grob("Scatterplot of empirical item EOI by predicted item EOI, No surprisal baseline", color="#3C3C3C", size=12, family="sans", face="bold"), bottom=text_grob("Predicted EOI", color="#3C3C3C", size=12, family="sans"), left=text_grob("Empirical EOI" , color="#3C3C3C", size=12, family="sans", rot=90))+theme_fivethirtyeight()
...
```{r EOI and baseline absolute RT, fig.width=3.5, fig.height=3.5}
ratio_display = 1
...
gp_corr <- baselinevsEOI_GP %>%
  mutate(paste("corr=",toString(round(CORR[[4]][[1]],3)), sep="")) %>%
  rename(est = `paste("corr=", toString(round(CORR[[4]][[1]], 3)), sep = "")`)
