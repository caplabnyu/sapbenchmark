...
#emp_dat <- readRDS('./saved_objects/rc_subset_sampledsumm_empirical')
...
saveRDS(by_construction, paste0('./plots/spr/RelativeClause/by_construction.rds'))
...
predicted_dat <- Predicting_RT_with_spillover(rt.data, 'RelativeClause')
...
i <- 1
for(m in unique(predicted_dat$model)){
  print(m)
  curr_rc <- subset(predicted_dat, model == m)
...
for(m in unique(pred_dat_verb$model)){
  ...
  saveRDS(curr_by_construction, paste0('./plots/spr/RelativeClause/by_construction_', m, '.rds'))
  ...
  saveRDS(curr_by_item, paste0('./plots/spr/RelativeClause/by_item_', m, '.rds'))
}


```

### Repeating with LMER models
...
create_dfs_lmer <- function(fit, model_name){
  ...
  saveRDS(by_construction, 
          paste0('./plots/spr/RelativeClause/by_construction_lmer_', model_name, '.rds'))
  ...
  saveRDS(by_item, 
          paste0('./plots/spr/RelativeClause/by_item_lmer_', model_name, '.rds'))
...
create_dfs_lmer(fit_verb_lmer_pred_gpt2, 'lstm')

fit_verb_emp_lmer <- readRDS('./saved_objects/fit_verb_lmer')

create_dfs_lmer(fit_verb_lmer_pred_gpt2, '')

```

**Correlating LMER and BRMS item level effects**
```{r}

by_item_lmer_gpt2 <- readRDS('./plots/spr/RelativeClause/by_item_lmer_gpt2.rds')
by_item_brms_gpt2 <- readRDS('./plots/spr/RelativeClause/by_item_gpt2.rds')

by_item_lmer_lstm <- readRDS('./plots/spr/RelativeClause/by_item_lmer_lstm.rds')
by_item_brms_lstm <- readRDS('./plots/spr/RelativeClause/by_item_lstm.rds')

cor(by_item_lmer_gpt2$mean, by_item_brms_gpt2$mean)
cor(by_item_lmer_lstm$mean, by_item_brms_lstm$mean)
