...
```{r}
spr$word <- str_replace(tolower(spr$EachWord), "[.,!?;:]", "") # lowercase and remove punctuation
...
Drop rows with missing data (surprisals for past 3 words and freqs for past 3 words) AND the last word of each sentence

```{r}

filler.with_lags$sent_length <- lapply(str_split(filler.with_lags$Sentence, " "), length)
...
filler.drop.lstm <- subset(filler.with_lags, !is.na(surprisal_lstm) & !is.na(surprisal_lstm_p1) & 
                                        ...
                                        !is.na(logfreq_p2) & !is.na(logfreq_p3) & (filler.with_lags$sent_length != filler.with_lags$WordPosition))
...
filler.drop.lstm = filler.drop.lstm %>% 
  mutate(surprisal_lstm_p1_s = (surprisal_lstm_p1 - surp_center)/surp_scale,
         surprisal_lstm_p2_s = (surprisal_lstm_p2 - surp_center)/surp_scale,
         surprisal_lstm_p3_s = (surprisal_lstm_p3 - surp_center)/surp_scale)
...
filler.drop.gpt2 <- filler.drop.gpt2 %>% 
  mutate(surprisal_gpt2_p1_s = (surprisal_gpt2_p1 - surp_center)/surp_scale,
         surprisal_gpt2_p2_s = (surprisal_gpt2_p2 - surp_center)/surp_scale,
         surprisal_gpt2_p3_s = (surprisal_gpt2_p3 - surp_center)/surp_scale)

filler.drop.lstm$logfreq_s <- scale(filler.drop.lstm$logfreq)
lf_center <- attributes(filler.drop.lstm$logfreq_s)$`scaled:center`
lf_scale <- attributes(filler.drop.lstm$logfreq_s)$`scaled:scale`

filler.drop.lstm$length_s <- scale(filler.drop.lstm$length)
l_center <- attributes(filler.drop.lstm$length_s)$`scaled:center`
l_scale <- attributes(filler.drop.lstm$length_s)$`scaled:scale`

filler.drop.lstm <- filler.drop.lstm %>%
    mutate(logfreq_p1_s = (logfreq_p1 - lf_center)/lf_scale,
           logfreq_p2_s = (logfreq_p2 - lf_center)/lf_scale,
           logfreq_p3_s = (logfreq_p3 - lf_center)/lf_scale,
           length_p1_s = (length_p1 - l_center)/l_scale,
           length_p2_s = (length_p2 - l_center)/l_scale,
           length_p3_s = (length_p3 - l_center)/l_scale)

filler.drop.gpt2$logfreq_s <- scale(filler.drop.gpt2$logfreq)
lf_center <- attributes(filler.drop.gpt2$logfreq_s)$`scaled:center`
lf_scale <- attributes(filler.drop.gpt2$logfreq_s)$`scaled:scale`

filler.drop.gpt2$length_s <- scale(filler.drop.gpt2$length)
l_center <- attributes(filler.drop.gpt2$length_s)$`scaled:center`
l_scale <- attributes(filler.drop.gpt2$length_s)$`scaled:scale`
filler.drop.gpt2 <- filler.drop.gpt2 %>%
    mutate(logfreq_p1_s = (logfreq_p1 - lf_center)/lf_scale,
           logfreq_p2_s = (logfreq_p2 - lf_center)/lf_scale,
           logfreq_p3_s = (logfreq_p3 - lf_center)/lf_scale,
...
models.filler.lstm <- lmer(data=filler.drop.lstm,
                      RT ~ surprisal_lstm_s + surprisal_lstm_p1_s + surprisal_lstm_p2_s + surprisal_lstm_p3_s +
                        ...
                        (1 + surprisal_lstm_s + surprisal_lstm_p1_s + surprisal_lstm_p2_s + surprisal_lstm_p3_s || participant) + (1 | item),
...
models.filler.gpt2 <- lmer(data=filler.drop.gpt2,
                      RT ~ surprisal_gpt2_s + surprisal_gpt2_p1_s + surprisal_gpt2_p2_s + surprisal_gpt2_p3_s +
                           scale(WordPosition) + logfreq_s*length_s + logfreq_p1_s*length_p1_s + 
                           logfreq_p2_s*length_p2_s + logfreq_p3_s*length_p3_s + 
                        (1 + surprisal_gpt2_s + surprisal_gpt2_p1_s + surprisal_gpt2_p2_s + surprisal_gpt2_p3_s || participant) + (1 | item),
                      control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
