...
surps_agr <- read.csv(paste0("../../../Surprisals/data/", model,"/items_Agreement.", model, ".csv.scaled"))
surps_npz <- read.csv(paste0("../../../Surprisals/data/", model,"/items_ClassicGP.", model, ".csv.scaled"))
...
rt.bmodel <- lmer(predicted_rt ~ pGram.coded * (position.coded.1 + position.coded.2) + (1 + pGram.coded * (position.coded.1 + position.coded.2) || item) + (1 + pGram.coded * (position.coded.1 + position.coded.2) || participant),
                 data = subset(rt.ht_data, Type == "AGREE"),
                 control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5))
)

saveRDS(rt.bmodel, paste0("models/agreement_lmodel_prior1_", model, "_predicted_agr.rds"))
