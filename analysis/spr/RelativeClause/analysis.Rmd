...
saveRDS(predicted_dat_rc, 'predicted_dat_rc.rds')

predicted_dat_filler <-  Predicting_RT_with_spillover(filler.data, 'filler')
saveRDS(predicted_dat_filler, 'predicted_dat_filler.rds')
...
for(m in unique(predicted_dat_rc$model)){
  print(m)
  curr_rc <- subset(predicted_dat_rc, model == m & !is.na(RT))
  curr_filler <- subset(predicted_dat_filler, model == m & !is.na(RT))
...
fit_verb_bayes <- brm(corrected_rt ~ Type_num +
                  ...
                  adapt_delta = brms_parms$adapt_delta
                  )
